...
class MetaModule(nn.Module):
    ...
    def update_params(self, lr_inner, source_params=None,
        ...
        if solver == 'sgd':
            ...
            # print(len(source_params), len(list(self.named_params(self))))
            # print(source_params == None)
            # i = 0
            for tgt, gradVal in zip(self.named_params(self), source_params):
                if gradVal is None: continue
                name_t, param_t = tgt
                ...
                self.set_param(self, name_t, nn.Parameter(newParam.cuda(), requires_grad=True))
