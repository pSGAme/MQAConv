...
def process_dir2(dir_paths, relabel=False):
    img_paths = []
...
def process_dir3(dir_paths, relabel=False):
    img_paths = []
    third_paths = []
    for i in range(2):
        img_paths.extend(glob.glob(osp.join(dir_paths[i], '*.jpg')))
    third_paths.extend(glob.glob(osp.join(dir_paths[2], '*.jpg')))
    pattern = re.compile(r'([-\d]+)_c(\d)')
    # get all identities
    pid_container = set()
    for img_path in img_paths:
    ...
    data = []
    for img_path in img_paths+third_paths:
        pid, camid = map(int, pattern.search(img_path).groups())
        if (pid not in pid_container) or (pid == -1):
            continue
...
class DukeMTMCreID_FULL(BaseImageDataset):
    ...
    def __init__(self, root, verbose=True):
        super(DukeMTMCreID_FULL, self).__init__()
        ...
        train = process_dir2(dir_paths = [self.train_dir, self.query_dir, self.gallery_dir], relabel=True)
