...
# root = /data1/ckx/data/market1501
...
class Market(object):
    ...
    def preprocess(self, path, relabel=True):
        ...
        all_camids = set()
        ret = []
        #print(osp.join(self.images_dir,path))
        ...
        for fpath in fpaths:
            ...
            all_camids.add(cam)
        return ret, int(len(all_pids)), len(all_camids)

    def load(self):
       # print(self.train_path)
        self.train, self.num_train_ids, self.num_cam_ids = self.preprocess(self.train_path)

        self.gallery, self.num_gallery_ids, _ = self.preprocess(self.gallery_path, False)
        self.query, self.num_query_ids, _ = self.preprocess(self.query_path, False)
