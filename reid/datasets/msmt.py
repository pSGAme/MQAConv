...
# root = /data1/ckx/data/msmt17

class MSMT(object):

    def __init__(self, root, combine_all=True):

        self.images_dir = osp.join(root, "MSMT17_V1")
    ...
    def preprocess(self, list_file):
        ...
        return data, int(len(all_pids)), len(all_camids)

    def load(self):
        self.train, self.num_train_ids, self.num_cam_ids = self.preprocess(self.train_list_file)
        self.val, self.num_val_ids, _ = self.preprocess(self.val_list_file)
        self.gallery, self.num_gallery_ids, _ = self.preprocess(self.gallery_list_file)
        self.query, self.num_query_ids, _ = self.preprocess(self.query_list_file)
